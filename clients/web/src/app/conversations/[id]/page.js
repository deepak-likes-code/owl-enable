...
const ConversationDetail = ({ params }) => {
  ...
  useEffect(() => {
    if (socket) {
      ...
      const handleNewUtterance = (newUtterance) => {
        ...
        if (
          newUtterance.conversation_uuid ===
          conversationRef.current?.conversation_uuid
        ) {
          ...
          setConversation((currentConversation) => {
            if (currentConversation) {
              const updatedConversation = {
                ...currentConversation,
                transcriptions: currentConversation.transcriptions.map(
                  (transcription) => {
                    return {
                      ...transcription,
  ...
  return (
    <div className="max-w-4xl mx-auto p-5">
                        ...
                        <div className="ml-2 flex-shrink-0 flex">
                          {utterance.start && (
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                              {utterance.start.toFixed(2)}s -{" "}
                              {utterance.end.toFixed(2)}s
                            </span>
                          )}
                        </div>
                      </div>
      ...
      {conversation.suggested_links && (
        <div className="mb-6">
          <h2 className="text-xl font-bold mb-4">Links</h2>
          <div className="grid grid-cols-2 gap-4">
            {conversation.suggested_links.map((link) => (
              <div className="col-span-1">
                {/* Container with fixed dimensions for consistency */}
                <div className=" flex justify-center items-center  bg-white shadow-lg rounded-lg">
                  {/* Adjust Microlink to fit within the container. Ensure it can adapt to the container's size, possibly through its own props or an embedded <iframe> or <img> tag. */}
                  <Microlink
                    url={link.url}
                    lazy={{ threshold: 0.5 }}
                    size="normal"
        ...
        <div className="mb-6">
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <div className="px-4 py-5 sm:px-6">
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                {conversation.primary_location.address}
              </p>
            </div>
            {googleMapsApiKey ? (
              <div className="px-4 py-5 sm:px-6">
              ...
              <div className="px-4 py-5 sm:px-6 bg-gray-50">
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Latitude: {conversation.primary_location.latitude}
                </p>
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Longitude: {conversation.primary_location.longitude}
                </p>
              </div>
            )}
          </div>
