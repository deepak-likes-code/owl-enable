            ...
            >
              <div className="p-6 rounded-lg border border-gray-700 flex justify-between items-center space-x-4">
                <div className="flex-1 min-w-0">
                  <h5 className="text-xl sm:text-2xl font-bold tracking-tight text-white truncate">
                    {conversation.short_summary}
                  </h5>
                  <p className="font-normal text-gray-400 mt-2">
                    {new Date(`${conversation.start_time}Z`).toLocaleString()}
                  </p>
                </div>
                {conversation.state === "CAPTURING" ? (
                  ...
                  <span
                    className={`px-4 py-1 rounded-full text-white ml-4 flex-shrink-0 ${getConversationStateStyle(
                      conversation.state
                    )}`}
                  >
